...
(async () => {
  const b = 64
  const map = new Array(b * b).fill([TerrainIndex['gras'], 0]).map((v, i) => [v[0], degToRad(Math.floor(random('' + i)() * 4)) * 90])
  ...
  map[55] = [TerrainIndex['trees'], degToRad(90)]
  map[56] = [TerrainIndex['trees'], degToRad(90)]
  map[57] = [TerrainIndex['trees'], degToRad(90)]
  map[58] = [TerrainIndex['stone'], degToRad(90)]
  map[120] = [TerrainIndex['clay'], degToRad(90)]
